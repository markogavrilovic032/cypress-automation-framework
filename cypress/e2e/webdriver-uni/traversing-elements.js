...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("nextAll() to get all of the next sibling DOM elements within elements", () => {
    cy.get('.traversal-drinks-list>[id="tea"]').nextAll().should('have.length', 3)
  });

  it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
    cy.get('.traversal-drinks-list>[id="tea"]').nextUntil().should('contain', 'Milk')
  });
  ...
  it("prev() to get the previous sibling DOM element within elements", () => {
    cy.get('.traversal-drinks-list>[id="sugar"]').prev().contains('Espresso')
  });

  it("prevAll() to get all previous sibling DOM elements within elements", () => {
    cy.get('.traversal-drinks-list>[id="sugar"]').prevAll().should('have.length', 4)
  });
